...
```{r}
cleaned_data %>% ggplot(aes(x = income_family)) + geom_bar(aes(fill = income_family)) + labs(title = "Figure 1: Distribution of Household Income", x = "Income group", y = "Count") + theme_gray()
cleaned_data %>% ggplot(aes(x = age))
cleaned_data %>% ggplot(aes(x = age))
cleaned_data %>% ggplot(aes(x = age))
