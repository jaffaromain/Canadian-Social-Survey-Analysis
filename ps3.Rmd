...
cleaned_data <- gss %>% 
  select(age, marital_status, income_family, own_rent, hh_size, province) %>% 
  filter(marital_status != "NA", province  != "NA", own_rent  != "NA", hh_size != "NA", own_rent != "Don't know") %>% 
  mutate(own_rent = ifelse(own_rent == "Rented, even if no cash rent is paid", 1, 0))

cleaned_data %>% as_tibble() %>% head()
```

To procure this data, we have used the script provided by Professor Rohan Alexander. This script loads in the raw GSS data, cleans it up, and leaves the final result for us to do our analysis. We're using the most recently done survey. This was held in 2017. This dataset is rich with several interesting variables that could be used for a variety of analyses. For our purposes, we've decided to select a few variables. Our variables of interest are:
...
For our purposes, we chose to use a Logistic regression model. The features of this model:
...
where the $\beta_n$ are the factored province, family income, size of household, and marital status respectively.
...
- This model was primarily chosen due to our choice of a binary response variable. Our response variable was **own_rent**. This is a categorical variable where 1 represents being an renter, and 0 being an owner. A simple linear regression wouldn't be suitable for this purpose.
...
The results are split into 2 sections, model results and survey graphs.

## Model Results

Through fitting a model using the chosen predictor variables, we are left with the given fitted logistic model with the following coefficients:
...
```
Figure 7 shows the distribution of household sizes when grouped by household income. We observe that respondents below a $50,000 income bracket are overwhelmingly one and 2 people households.
...
- As mentioned in the introduction, this data represents an interesting snapshot of time. As this survey was conducted in 2017, this represents our pre-covid world. When this survey is conducted again in 2022, it'll be interesting to remodel these relationships and see how they have changed due to a significant event. We suspect that since remote work has become significantly more acceptable, with multiple large companies letting employees work from home permanently even once things return to normal, families are more likely to own a house. This conclusion is based on several factors. Since there is no requirement to remain close to corporate hubs such as Toronto/Vancouver, families can now move out of the city to cheaper cities. This reduces the cost barrier as illustrated in the following point. This effect could trickle down in many ways, for instance, we might see an increased number of owners in some income brackets. There is also a chance that this might not be nearly as pronounced, the acceptance of remote work at a permanent level may still only be acceptable to those working in industries that can function without minimal contact. The survey does track the industry that the respondent works in. Using this information, it would be interesting to see if there any trends in the distribution of home-owners by industry in a pre and post covid world. 
...
Lüdecke D (2020). _sjPlot: Data Visualization for Statistics in Social Science_. R package version 2.8.5, <URL:
